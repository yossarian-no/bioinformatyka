...
def greedy_build(probes, n, k, start_probe=None):
    if not probes:
        return list('ACGT' * (n//4+1))[:n]

    if start_probe and start_probe in probes:
        start = start_probe
    else:
        # Fall back: find probe with no incoming edges
        starts = set(probes)
        for p in probes:
            for q in probes:
...
def multi_start(probes, n, k, start_probe=None, restarts=5):
    best_dna, best_score = None, -1
    for i in range(restarts):
        sh = probes[:]
        random.shuffle(sh)
        # On first restart always try the real start probe first
        dna = greedy_build(sh, n, k, start_probe=(start_probe if i == 0 else None))
...
def main():
    ...
    if start_seq:
        ...
        if start_probe:
            print(f"[INFO] Start probe found: {start_probe}")
        else:
            print("[INFO] Start probe not found, trying all probes")

    result = ''.join(multi_start(probes, n, k, start_probe=start_probe)[:n])
    print(result)

    with open("output_heur.txt", "w") as out:
        out.write(result + "\n")
    print(f"[INFO] Result saved to output_heur.txt (length: {len(result)})")
